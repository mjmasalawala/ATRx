...
Only candidates are kept: `addCandidatesFromRun()` flattens every `is_candidate` level from a tier's `screening_breakdown` into one row each (tagging it with `tier` and that run's `generated_at`), appended to a single `CANDIDATES` array spanning all tiers so far and re-rendered (sorted by `score` descending) after every tier completes — not held back until the whole run finishes. A `PROGRESS` array (one entry per tier: pending/running/done/error) renders as chips above the table so the in-flight state is visible tier by tier. A 401 (not logged in) on any tier stops the whole run early, since every remaining tier would fail the same way; any other per-tier error is shown in that tier's chip and the run continues to the next tier after the usual 30s gap.
