...
**Gotcha (forward-day column labels):** the backend's JSON field names `hit_rate_3d_pct`/`avg_fwd_ret_3d_pct`/`hit_rate_5d_pct`/`avg_fwd_ret_5d_pct` (`screener.py`) are fixed strings baked in regardless of the actual `forward_days_short`/`forward_days_long` values used for a run -- those are user-tunable, so a run with e.g. `forward_days_short=2` still comes back keyed `hit_rate_3d_pct`. The frontend never shows that literal key as a label: `columnLabel()` (in `index.html`, `atrx_stock.html`, and `find_atrx_opps.html`) rewrites those four columns' header text to `Hit% ${LAST_FORWARD_DAYS_SHORT}d` etc. at render time, where `LAST_FORWARD_DAYS_SHORT`/`LAST_FORWARD_DAYS_LONG` are captured from the `overrides` actually sent for that run (attached onto the response as `data._forward_days_short`/`_forward_days_long` right after the fetch, so they travel with the cached result too) -- not re-read from the live config form, which could have changed since the run completed.
...
## `find_atrx_opps.html` — Find ATRx Opps page

Same config-panel scaffolding/CSS as index.html, minus the tier `<select>` (this page always runs every tier). "Run All" sequentially `POST`s `/api/run-screener` once per tier from `GET /api/universe-tiers` (falls back to a hardcoded `large_cap`/`mid_cap`/`small_cap` list if that call fails), waiting `RUN_GAP_MS` (30s) between calls to stay clear of Kite's per-second rate limit — this is why it's a client-side loop across several short requests rather than one server-side call: a single request looping over all tiers would very likely blow past Vercel's 60s function timeout (`vercel.json`'s `functions.api/index.py.maxDuration`) once the per-tier Kite calls plus the deliberate 30s gaps are added up.

Only candidates are kept: `addCandidatesFromRun()` flattens every `is_candidate` level from a tier's `screening_breakdown` into one row each (tagging it with `tier` and that run's `generated_at`), appended to a single `CANDIDATES` array spanning all tiers so far — this is the raw list, one entry per qualifying *level*, and it's what gets cached. A `PROGRESS` array (one entry per tier: pending/running/done/error) renders as chips above the table so the in-flight state is visible tier by tier. A 401 (not logged in) on any tier stops the whole run early, since every remaining tier would fail the same way; any other per-tier error is shown in that tier's chip and the run continues to the next tier after the usual 30s gap.

Display, not the raw list, is deduped: a symbol can have more than one qualifying level in the same run (or reappear in another tier, if universes overlap), but only one is worth acting on for entry. `renderCandidates()` calls `bestLevelPerSymbol(CANDIDATES)` to keep just the highest-`score` level per symbol before sorting and rendering — so the table shows at most one row per symbol, even though `CANDIDATES` (and the cache) still holds every qualifying level found. The `Trigger Level` column (`lvl.level`, i.e. `round(lvl.price, 2)` from `screener.py`) is the actual support price a candidate is near — distinct from `Price`, the current market price — and is what tells you where to consider entry.

Because this is plain client-side `setTimeout`/`fetch` sequencing (no server-side job), the run only keeps going as long as the browser tab stays open and the script keeps executing — backgrounding the *tab* (switching to another tab or application) is fine, browsers don't suspend `fetch`/`setTimeout` in a background tab, just throttle timer granularity slightly. Navigating *away from this page* (clicking a different sidebar link in the same tab) does stop it, same as closing the tab would — there's no backend job queue behind this, by design, to stay within the existing per-request Vercel timeout model.

Same cache-then-refresh pattern as the rest of the app ([[feedback_cache_then_refresh_pattern]] in this project's Claude memory): the last completed run's `CANDIDATES`/`PROGRESS` are cached in `localStorage` (`atrx_find_opps_cache_v1`) and restored on page load without auto-running; "Run All" is the only thing that triggers a real run.
