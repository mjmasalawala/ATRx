...
- `_TRADE_ORDER_BY = "trade_time::date ASC, (side = 'SELL') ASC, trade_time ASC, trade_id ASC"` — grouped by **calendar day** first, every BUY on a day replayed before that day's SELLs regardless of intraday order, since Zerodha's export can log a same-day SELL (e.g. an existing lot sold that morning) with an earlier `order_execution_time` than that day's BUY (e.g. a fresh lot bought that afternoon) -- sorting by raw `trade_time` alone would put the SELL first and `replay()` would see a false oversell against zero held shares. BUY-before-SELL doesn't affect that day's P&L under average-cost (unlike FIFO); `trade_time` is the next tiebreak (same-side same-day trades keep their real execution order), `trade_id` the final one. Reordering only ever happens *within* a day, never across days.
