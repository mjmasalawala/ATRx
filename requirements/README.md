...
- [atrx-screener.md](atrx-screener.md) — the core support-level bounce strategy: `config.py`/`indicators.py`/`levels.py`/`backtest.py`/`screener.py`/`report.py`, the WhatsApp candidate-alert cron (`candidate_alert.py`, `notify.py`, `whatsapp_notify.py`, `nse_holidays_sync.py`), `blob_store.py`, and the `index.html`/`atrx_stock.html`/`find_atrx_opps.html` frontend pages.
- [breakout-screener.md](breakout-screener.md) — a second, independent strategy: the "N-of-M block staircase" breakout signal (`breakout_signal.py`/`breakout_config.py`/`breakout_filters.py`/`breakout_screener.py`/`breakout_stats.py`) and its own persisted evidence tables, with `breakout.html` as the frontend page.
